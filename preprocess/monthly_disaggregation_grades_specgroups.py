...
# Dosyayı oku
file_path = "/Users/ceylin/Desktop/indr491/indr491/preprocessedBelgeler/forecastData_altLimit100_son3_byGrup.xlsx"
...
df['Total_Grade_Month'] = df.groupby(['YearMonth', 'Grade'])['Sipariş Mik. (TON)'].transform('sum')
...
for grade in unique_grades:
    ...
    unique_spec_groups = grade_data['SpecGroupId'].unique()

    for spec_group in unique_spec_groups:
        plt.figure(figsize=(12, 6))
        spec_group_data = grade_data[grade_data['SpecGroupId'] == spec_group].sort_values(by="YearMonth")  # Tarihe göre sıralama
        
        plt.plot(spec_group_data['YearMonth'], spec_group_data['Percentage'], marker='o', linestyle='-')
        plt.title(f'SpecGroupId {spec_group} - Percentage Distribution within {grade}')
